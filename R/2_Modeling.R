...
#### Set the outcome ####
sev$outcomeCVD <- ifelse(sev$outcomeCVD == 1, "yes", "no")
positiveClass <- "yes"
outcomeName = "outcomeCVD" #The name of column
...
sevCat <- sev %>% select(all_of(catVar))
sevNum <- sev %>% select(all_of(numVar))
sevNumR <- as.data.frame(lapply(sevNum, zNormalize))

#### One hot encoding ####
for(col in colnames(sevCat)){
  sevCat[,col] <- as.character(sevCat[,col] )
}

dummy <- caret::dummyVars("~ .", data = sevCat) 
sevCatD <- data.frame(predict(dummy, newdata = sevCat))

#feature <- sevCatD
feature <- cbind(sevCatD, sevNumR)
